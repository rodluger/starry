        ...
        if ((b >= 1.0 + r) ||  (r == 0.0)) {
            ...
            if (b == 0) {
                ...
                if (r == 0.5) {
                    ...
                    m = 4 * r2;
                    Lambda1 = (1. / 3.) + 2. / (9. * PI<T>()) * ellip::CEL(m, T(sqrt(1.0 - m)), T(1), T(m - 3), T((1 - m) * (2 * m - 3)));
                } else {
                    m = 4 * r2;
                    minv = 1.0 / m;
                    Lambda1 = (1. / 3.) + 1. / (9. * PI<T>() * r) * ellip::CEL(minv, T(sqrt(1.0 - minv)), T(1), T(m - 3), T(1 - m));
                }
            } else {
                ...
                if ((b + r) > 1.0) {
                    ...
                    if (s2_unstable(b, r)) {
                        k2c = -onembpr2 / fourbr;
                        kc = sqrt(k2c);
                        Lambda1 = onembmr2 * (k2c * bmr * bpr * ellip::CEL(k2, kc, T(bmr2 * k2c), T(0), T(3))
                                  + ellip::CEL(k2, kc, T(1), T(-3 + 6 * r2 - 2 * br), onembpr2)) / (9. * PI<T>() * sqrt(br));

                    } else {
                        Lambda1 = ((bpr2 - 1) / bpr * (-2 * r * (2 * bpr2 - bpr * bmr - 3) * K + PiTerm(b, r, ksq, bmr, bmr2, bpr2, bmrdbpr)) - 2 * xi * E) / (9 * PI<T>() * sqrt(br));
                    }
                } else if ((b + r) < 1.0) {
                    // k^2 > 1
                    if (s2_unstable(b, r)) {
                        ...
                        Lambda1 = 2 * sqrt(onembmr2) * (onembpr2 * ellip::CEL(k2inv, kc, p, T(1 + mu), T(p + mu)) - (4 - 7 * r2 - b2) * Eofk) / (9. * PI<T>());
                    } else {
                        Lambda1 = 2 * ((1 - bpr2) * (sqrt(1 - bmr2) * K + PiTerm(b, r, ksq, bmr, bmr2, bpr2, bmrdbpr)) - sqrt(1 - bmr2) * (4 - 7 * r2 - b2) * E) / (9 * PI<T>());
                    }
                } else {
                    // b + r  = 1 or k^2 = 1
                    Lambda1 = 2. / (3. * PI<T>()) * acos(1. - 2. * r) - 4 / (9. * PI<T>()) * (3 + 2 * r - 8 * r2) * sqrt(br) - (2. / 3.) * T(r > 0.5);
                }
            }
        }
        return Lambda1;
