...
def _retrieve_online_data(guess_masses=False):
    ...
    try:
        ...
        planets = pd.read_csv(NEXSCI_API + '?table=planets&select=pl_hostname,pl_letter,'
                            ...
                            'st_raderr2,st_teff,st_tefferr1,st_tefferr2,st_optmag,st_j,st_h', comment='#')
        # Composite table
        composite = pd.read_csv(NEXSCI_API + '?table=compositepars&select=fpl_hostname,fpl_letter,'
                            ...
                            'fpl_eccen,fpl_eccenerr1,fpl_eccenerr2,fst_spt', comment='#')
        # Rename columns
        composite.columns = ['pl_hostname','pl_letter','pl_orbsmax','pl_orbsmaxerr1',
                            ...
                            'pl_eqt','pl_orbper','pl_orbpererr1','pl_orbpererr2',
                            'pl_eccen','pl_eccenerr1','pl_eccenerr2','st_spt']
    except:
