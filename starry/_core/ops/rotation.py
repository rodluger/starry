...
__all__ = ["dotROp", "tensordotRzOp", "tensordotDzOp"]
...
class tensordotDzOp(tt.Op):
    def __init__(self, func):
        self.func = func
        self._grad_op = tensordotDzGradientOp(self)
...
class tensordotDzGradientOp(tt.Op):
    def __init__(self, base_op):
        self.base_op = base_op
    ...
    def perform(self, node, inputs, outputs):
        bM, btheta, balpha = self.base_op.func(*inputs)
        outputs[0][0] = np.reshape(bM, np.shape(inputs[0]))
        outputs[1][0] = np.reshape(btheta, np.shape(inputs[1]))
        outputs[2][0] = np.reshape(balpha, np.shape(inputs[2]))
