...
\brief Sturm's theorem to ensure polynomial is positive in [0, 1].
       Adapted from DFM's celerite <https://github.com/dfm/celerite>

*/

#ifndef _STARRY_STURM_H_
#define _STARRY_STURM_H_
...
template <typename T>
inline Eigen::Matrix<T, Eigen::Dynamic, 1> polyadd(
    const Eigen::Matrix<T, Eigen::Dynamic, 1>& p1,
    const Eigen::Matrix<T, Eigen::Dynamic, 1>& p2) {
  int n1 = p1.rows(), n2 = p2.rows(), n = std::max(p1.rows(), p2.rows());
...
template <typename T>
inline Eigen::Matrix<T, Eigen::Dynamic, 1> polymul(
    const Eigen::Matrix<T, Eigen::Dynamic, 1>& p1,
    const Eigen::Matrix<T, Eigen::Dynamic, 1>& p2) {
  int n1 = p1.rows(), n2 = p2.rows(), n = n1 + n2 - 1;
  Eigen::Matrix<T, Eigen::Dynamic, 1> result(n);
  result.setConstant(0.0);
  for (int i = 0; i < n; ++i)
    for (int j = 0; j < n2; ++j)
      if (i - j >= 0 && i - j < n1 && j >= 0) result[i] += p1[i - j] * p2[j];
  return result;
}

template <typename T>
inline Eigen::Matrix<T, Eigen::Dynamic, 1> polyrem(
    const Eigen::Matrix<T, Eigen::Dynamic, 1>& u,
    const Eigen::Matrix<T, Eigen::Dynamic, 1>& v) {
  int m = u.rows() - 1, n = v.rows() - 1, p = m - n + 1;
  using std::abs;
  T d, scale = T(1.0) / v[0];
  Eigen::Matrix<T, Eigen::Dynamic, 1> q(std::max(p, 1)),
...
template <typename T>
inline Eigen::Matrix<T, Eigen::Dynamic, 1> polyder(
...
template <typename T>
inline std::vector<Eigen::Matrix<T, Eigen::Dynamic, 1>> polysturm(
    const Eigen::Matrix<T, Eigen::Dynamic, 1>& p) {
  int n = p.rows() - 1;
