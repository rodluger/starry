...
namespace orbital {
    ...
    //
    template <class T>
    Vector<T> System<T>::integrate (const T& time) {
        if (exptime > 0.0) {
            T dt = 0.5 * exptime,
              t1 = time - dt,
              t2 = time + dt;
    ...
    template <class T>
    Vector<T> System<T>::compute(const T& time, bool store_xyz) {
