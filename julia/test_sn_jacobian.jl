...
#for i=1:2
  ...
  if r < 1.0
    b = [linspace(1e-8,epsilon,nb); linspace(epsilon,delta,nb); linspace(delta,r-delta,nb);
     ...
     linspace(r+delta,1-r-delta,nb); linspace(1-r-delta,1-r-epsilon,nb); linspace(1-r-epsilon,1-r,nb);linspace(1-r,1-r+epsilon,nb);
...
#      ax[:semilogy](abs.(sn_jac_grid[:,n+1,1]-sn_jac_grid_num[:,n+1,1]),lw=1,label=string("l=",l))
    else
...
for n=0:n_max
  ...
  mask = abs.(diff) .> abs.(sn_jac_grid[:,n+1,:])
  if sum(mask) > 0 || mod(l-m,4) == 0
    println("n: ",n," max dsn/dr: ",maximum(abs.(diff)))
    println("b: ",b[mask[:,1]],b[mask[:,2]]," k^2: ",k2[mask[:,1]],k2[mask[:,2]])
# end
#end
#read(STDIN,Char)
