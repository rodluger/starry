...
def test_evaluation():
    ...
    assert np.allclose(I, [0.7736072493369371,
                           1.0432785526935853,
                           1.318434613210305])
    I, dI = map.evaluate(x=[0.1, 0.2, 0.3], y=[0.1, 0.2, 0.3], theta=30,
                         gradient=True)
    assert np.allclose(I, [0.7736072493369371,
                           1.0432785526935853,
                           1.318434613210305])
    ...
    assert np.allclose(I, [1.7026057774431276,
                           ...
                           0.7736072493369371,
                           1.7026057774431276])
    I, dI = map.evaluate(x=0.1, y=0.1, theta=[0, 30, 30, 0], gradient=True)
    assert np.allclose(I, [1.7026057774431276,
                           0.7736072493369371,
                           0.7736072493369371,
                           1.7026057774431276])
