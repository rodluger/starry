...
def test_intensity(abs_tol=1e-5, rel_tol=1e-5, eps=1e-7):
    ...
    f = [np.pi]
    verify_grad(map.ops.intensity, (xpt, ypt, zpt, y, u, f),
                abs_tol=abs_tol, rel_tol=rel_tol, eps=eps, n_tests=1)
...
def test_F(abs_tol=1e-5, rel_tol=1e-5, eps=1e-7):
    ...
    verify_grad(map.ops.F, (u, f), abs_tol=abs_tol, rel_tol=rel_tol, 
                eps=eps, n_tests=1)
...
def test_pT(abs_tol=1e-5, rel_tol=1e-5, eps=1e-7):
    ...
    y = 0.25
    z = np.sqrt(1 - x ** 2 - y ** 2)
    verify_grad(map.ops.pT, (x, y, z), abs_tol=abs_tol, 
...
def test_intensity_reflected(abs_tol=1e-5, rel_tol=1e-5, eps=1e-7):
    ...
    verify_grad(map.ops.intensity, (xpt, ypt, zpt, y, u, f, source),
                abs_tol=abs_tol, rel_tol=rel_tol, eps=eps, n_tests=1)
...
def test_rv(abs_tol=1e-5, rel_tol=1e-5, eps=1e-7):
    map = starry.Map(ydeg=2, doppler=True)
    theta = np.linspace(0, 30, 10)
    xo = np.linspace(-1.5, 1.5, len(theta))
    yo = np.ones_like(xo) * 0.3
    zo = 1.0 * np.ones_like(xo)
    ...
    # Just rotation
    verify_grad(map.ops.rv, (theta, xo, yo, zo, 0.0, inc, obl, y, u, veq, alpha), 
                abs_tol=abs_tol, rel_tol=rel_tol, eps=eps, n_tests=1)

    # Just occultation
    verify_grad(map.ops.rv, (theta, xo / 3, yo, zo, ro, inc, obl, y, u, veq, alpha), 
                abs_tol=abs_tol, rel_tol=rel_tol, eps=eps, n_tests=1)

    # Rotation + occultation
    verify_grad(map.ops.rv, (theta, xo, yo, zo, ro, inc, obl, y, u, veq, alpha), 
                abs_tol=abs_tol, rel_tol=rel_tol, eps=eps, n_tests=1)
