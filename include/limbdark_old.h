    ...
    */
    ...
    inline void GreensLimbDark<T>::computeIcoeffs () {
        ...
        for (int n = 1; n <= STARRY_IJ_MAX_ITER; ++n) {
            Icoeff[n] = (2.0 * n - 1.0) * 0.5 * (2 * n + 2 * ivmax - 1) / 
                        (n * (2 * n + 2 * ivmax + 1));
        }
    }
    ...
    inline void GreensLimbDark<T>::computeJ (
        ...
        if ((ksq < 0.5) || (ksq > 2)) {
            ...
            for (v = jvmax - 2; v >= 0; --v) {
                ...
                if (gradient)
                    dJdk[v] = (dJdk[v + 1] + (3.0 * k * invksq) * J[v + 1]) * 
                              invksq;
            }
        ...
        // Upward recursion
            ...
            // First two values
            if (ksq < 1) {
                J[0] = 2.0 / (3.0 * ksq * k) * (ksq * (3.0 * ksq - 2.0) * 
                       Em1mKdm + ksq * Eofk);
                J[1] = 2.0 / (15.0 * ksq * k) * (ksq * (4.0 - 3.0 * ksq) * 
                       Eofk + ksq * (9.0 * ksq - 8) * Em1mKdm);
                if (gradient) {
                    dJdk[0] = 2.0 / ksq * (-Eofk + 2 * Em1mKdm);
                    dJdk[1] = -3.0 * k * invksq * J[1] + ksq * dJdk[0];
                }
            } else {
                J[0] = (2.0 / 3.0) * ((3.0 - 2.0 * invksq) * 
                       Eofk + invksq * Em1mKdm);
                J[1] = 0.4 * (1.0 / 3.0) * ((-3.0 + 4.0 * invksq) *     
                       Em1mKdm + (9.0 - 8.0 * invksq) * Eofk);
                if (gradient) {
