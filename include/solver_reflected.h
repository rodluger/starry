    ...
    */
    ...
    protected:
        ...
        */
        ...
        inline void computeHI_with_grad (
            const Scalar& bterm
        ) {
            H.setZero();
            I.setZero();
            ...
            for (int j = 0; j < lmax + 1; ++j) {
                I(j + 2) = Scalar(1.0) / Scalar(j + 4.0) * 
                           (fac1 + (j + 1) * I(j));
                DIDb(j + 2) = Scalar(1.0) / Scalar(j + 4.0) * 
                           (Dfac1Db + (j + 1) * DIDb(j));
                H(j + 2) = 0.5 * (Scalar(1.0) - fac2);
    ...
    public:
        ...
        */
        inline Scalar compute( 
            const Scalar& bterm,
            const RowVector<Scalar>& brT
        ) {
            Scalar bb = 0.0;
            ...
            for (int l = 0; l < lmax + 1; ++l) {
                for (int m = -l; m < l + 1; ++m) {
                    ...
                    if (is_even(nu)) {
                        ...
                        j = nu / 2;
                        bb += ((DfacDb * H(j + 1) + 
                                fac * DHDb(j + 1)) * J(i, j + 1) -
                                (I(j) + bterm * DIDb(j)) * K(i, j)) * brT(n);
                    } else {
                        i = (mu - 1) / 2;
                        j = (nu - 1) / 2;
                        bb += ((DfacDb * I(j + 1) + 
                                fac * DIDb(j + 1)) * K(i, j + 1) - 
                                ((H(j) * J(i, j) - H(j) * J(i + 2, j) - 
                                  H(j + 2) * J(i, j + 2)) +
                                 bterm * (DHDb(j) * J(i, j) - 
                                 DHDb(j) * J(i + 2, j) - 
                                 DHDb(j + 2) * J(i, j + 2)))) * brT(n);
                    }
                    ++n;
                }
            }
